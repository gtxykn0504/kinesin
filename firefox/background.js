...
function broadcastSyncStatus(status, message) {
  browser.runtime.sendMessage({ action: 'syncStatusUpdate', status, message }).catch(() => {});
}
...
browser.runtime.onStartup.addListener(handleExtensionStart);
browser.runtime.onInstalled.addListener(handleExtensionStart);
